...
int main(int argc, char **argv)
{
  ...
  std::string sOutDir = "";
  std::string posfile;
  int iRotationAveragingMethod = int (ROTATION_AVERAGING_L1);
  int iTranslationAveragingMethod = int (TRANSLATION_AVERAGING_L2_DISTANCE_CHORDAL);
  std::string sIntrinsic_refinement_options = "ADJUST_ALL";
  bool b_use_motion_priors = false;
  /*std::string dataset = "E:/xiao.teng/sfmIPI/dataset/16images";
  sSfM_Data_Filename = dataset + "/sfme.json/sfm_data.json";
  sMatchesDir = dataset+"/sfme.json";
  sMatchFilename = dataset + "/sfme.json/matches.e.bin";
  sOutDir = dataset + "/sfme.json/sfm_pos";
  posfile = dataset + "/POS/posCR_n.txt";*/
  
  
  cmd.add(make_option('g', posfile, "posfile"));
  ...
  GlobalSfMReconstructionEngine_RelativeMotions sfmEngine(
    sfm_data,
    sOutDir,
    stlplus::create_filespec(sOutDir, "Reconstruction_Report_global.html"));
  ...
  if (sfmEngine.Process_pos(posfile))
  {
    std::cout << std::endl << " Total Ac-Global-Sfm took (ms): " << timer.elapsedMs() << std::endl;

    std::cout << "...Generating SfM_Report.html" << std::endl;
    Generate_SfM_Report(sfmEngine.Get_SfM_Data(),
      stlplus::create_filespec(sOutDir, "SfMReconstruction_Report_global.html"));

    //-- Export to disk computed scene (data & visualizable results)
    std::cout << "...Export SfM_Data to disk." << std::endl;
    Save(sfmEngine.Get_SfM_Data(),
      stlplus::create_filespec(sOutDir, "sfm_data_afterBA", ".bin"),
      ESfM_Data(ALL));

    Save(sfmEngine.Get_SfM_Data(),
      stlplus::create_filespec(sOutDir, "cloud_and_poses_global", ".ply"),
      ESfM_Data(ALL));
