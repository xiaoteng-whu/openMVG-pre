...
bool GlobalSfMReconstructionEngine_RelativeMotions::Process() {
  ...
  // output some intermediate sfm_data_*.bin
  //-------------------
  {
...
bool GlobalSfMReconstructionEngine_RelativeMotions::ProcessIO(std::string dir_json) {
	...
	Save(sfm_data_,
		stlplus::create_filespec(dir_json, "sfm_data_RT_initial", ".bin"),
		ESfM_Data(ALL));
	if (!Compute_Initial_Structure(tripletWise_matches))
	{
		std::cerr << "GlobalSfM:: Cannot initialize an initial structure!" << std::endl;
	...
	Save(sfm_data_,
		stlplus::create_filespec(dir_json, "sfm_data_BA_initial", ".bin"),
		ESfM_Data(ALL));
	if (!Adjust())
	{
		std::cerr << "GlobalSfM:: Non-linear adjustment failure!" << std::endl;
...
bool GlobalSfMReconstructionEngine_RelativeMotions::Process_gesfm(std::string gesfmfile) {
	...
	//get intial R t from gesfm file
	//write these R t to sfm_data_
	std::ifstream in(gesfmfile);
	size_t Ncam = sfm_data_.views.size();
	Mat gesfm(Ncam, 12);
	...
	//
	for (size_t I = 0; I < Ncam; I++) {
		const View *view_I = sfm_data_.views.at(I).get();
		size_t poseid = view_I->id_pose;
		geometry::Pose3 ptmp;
		ptmp.rotation()(0, 0) = gesfm(I, 0);
		ptmp.rotation()(0, 1) = gesfm(I, 1);
		ptmp.rotation()(0, 2) = gesfm(I, 2);
		ptmp.rotation()(1, 0) = gesfm(I, 3);
		ptmp.rotation()(1, 1) = gesfm(I, 4);
		ptmp.rotation()(1, 2) = gesfm(I, 5);
		ptmp.rotation()(2, 0) = gesfm(I, 6);
		ptmp.rotation()(2, 1) = gesfm(I, 7);
		ptmp.rotation()(2, 2) = gesfm(I, 8);
		//ptmp.rotation() = ptmp.rotation().transpose();
		Vec3 C;
		C(0) = gesfm(I, 9);
		C(1) = gesfm(I, 10);
		C(2) = gesfm(I, 11);
		ptmp.center() = C;
		sfm_data_.poses[view_I->id_pose] = ptmp;
	}
...
bool GlobalSfMReconstructionEngine_RelativeMotions::Process_pos(std::string posfile) {
	...
	for (size_t I = 0; I < Ncam; I++) {
		...
		ptmp.rotation()(0, 0) = pos(I, 3);
		ptmp.rotation()(0, 1) = pos(I, 4);
		ptmp.rotation()(0, 2) = pos(I, 5);
		ptmp.rotation()(1, 0) = pos(I, 6);
		ptmp.rotation()(1, 1) = pos(I, 7);
		ptmp.rotation()(1, 2) = pos(I, 8);
		ptmp.rotation()(2, 0) = pos(I, 9);
		ptmp.rotation()(2, 1) = pos(I, 10);
		ptmp.rotation()(2, 2) = pos(I, 11);
