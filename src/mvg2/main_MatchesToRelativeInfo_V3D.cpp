...
int main(int argc, char **argv)
{
	...
	/////
	/*std::string sMatchesDir = "E:/xiao.teng/sfmIPI/2viewgraph_selection/expriements/test_capitole/sfm.json";
	std::string sOutFile = "E:/xiao.teng/sfmIPI/2viewgraph_selection/expriements/test_capitole/imagesV3D/relaOrient.txt";
	...
	{
		{
			...
			else
			{
				outrp << I << " " << J << " "
					<< Ngf << " "<< Sr << " " 
					<< 0.5*(Sd1+Sd2) << " " << 0.5*(A1+A2) << " "<< Sr*2.0/(A1 + A2)<<" "
				...
				{
					//outrp << x1(i, 0) <<" "<<x1(i,1)<< " " << x2(i, 0)<<" "<<x2(i,1) << std::endl;
					Vec2 x1 = cam_I->get_ud_pixel(
						feats_provider->feats_per_view.at(I)[matches[i].i_].coords().cast<double>());
					Vec2 x2 = cam_J->get_ud_pixel(
						feats_provider->feats_per_view.at(J)[matches[i].j_].coords().cast<double>());
					outrp << matches[i].i_ << " " << x1(0) << " " << x1(1) << " " << matches[i].j_ << " " << x2(0) << " " << x2(1) << std::endl;
				}
