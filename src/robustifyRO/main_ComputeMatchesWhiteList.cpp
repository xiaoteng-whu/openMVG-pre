...
int main(int argc, char ** argv)
{
	...
	cmd.add(make_option('w', sWhiteList, "sWhiteList.txt"));
	cmd.add(make_option('o', sOutFile, "sOutFile"));
	///
	/*std::string dataset = "E:/xiao.teng/sfmIPI/2viewgraph_selection/SeqVS/dataset/street";
	sSfM_Data_Filename = dataset+ "/sfm.json/sfm_data.json";
	sMatchesDir = dataset + "/sfm.json";
	sWhiteList = dataset + "/seqvs/whitelist_street.txt";
	sOutDir = dataset + "/seqvs";
	...
	// ------------
	std::ifstream white(sWhiteList);
	std::vector<Pair> whiteList;
	while (!white.eof())
	{
		IndexT i, j;
		white >> i >> j;
	...
	for (int i = 0; i < (int)map_GeometricMatches.size(); i++)
	{
		...
		vector<Pair>::const_iterator p = find(whiteList.begin(), whiteList.end(), current_pair);
		if (p != whiteList.end())
		{
